...
async def run_single_cycle():
    ...
    for event in events_list:
        ...
        try:
            ...
            # تشغيل توليد الذكاء الاصطناعي في Thread منفصل لمنع تجميد الـ Event Loop
            ...
            if reply_text:
                ...
                # حماية عملية النشر بحساب مهلة زمني لمنع التعليق
                try:
                    await asyncio.wait_for(client.send_event_builder(builder), timeout=10)
                    replies_count += 1
                    ...
                    print("Publish timeout, moving to next post...")
                except Exception as pub_err:
                    print(f"Error publishing event: {pub_err}")
            ...
            print(f"Skipping event error: {loop_err}")
            continue
